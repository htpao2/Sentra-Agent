...
let stopwordsIso = null; try { stopwordsIso = require('stopwords-iso'); } catch {}
let BadWordsFilter = null; try { BadWordsFilter = require('bad-words'); } catch {}
...
const segment = new Segment(); useDefault(segment);

const clamp = (x, a, b) => Math.max(a, Math.min(b, x));
const sigmoid = (z) => 1 / (1 + Math.exp(-z));
const isLikelyChinese = (t) => /[\u4e00-\u9fff]/.test(t);
const isLikelyLatin = (t) => /[A-Za-z]/.test(t);
const isNum = (x) => typeof x === 'number' && Number.isFinite(x);

function toHalfWidth(str) {
  return (str || '').replace(/[\uFF01-\uFF5E]/g, c => String.fromCharCode(c.charCodeAt(0) - 0xFEE0));
}
function normalizeForMatch(str) { return toHalfWidth(str).toLowerCase().replace(/\s+/g, ' ').trim(); }

// ---------------- resources & rules ----------------
function compileIntentRules(rules) {
  ...
  function score(text, lang) {
    ...
    for (const r of compiled) {
      ...
      try {
        let hit = 0;
        if (r.kind === 'regex') hit = r.regex.test(text) ? 1 : 0;
        else if (r.kind === 'fn') hit = clamp(r.fn(text, lang) || 0, 0, 1);
        if (hit > 0) probNoHit *= (1 - r.weight);
      } catch {}
    }
    return clamp(1 - probNoHit, 0, 1);
  }
  function matched(text, lang) {
    const m = [];
    for (const r of compiled) {
      if (r.lang !== '*' && r.lang !== lang) continue;
      try {
        if (r.kind === 'regex' && r.regex.test(text)) m.push({ type: 'regex', weight: r.weight, pattern: r.pattern });
        if (r.kind === 'fn') {
...
function buildStopwords(config) {
  let en = new Set();
...
// tokenization
function englishTokensAll(text) { const doc = nlp(text); return doc.terms().out('array') || []; }
function englishTokensNoStop(text, stopSet) {
  const ts = englishTokensAll(text);
  return ts.map(t => t.toLowerCase()).filter(t => t && !stopSet.has(t) && !/^\d+$/.test(t));
}
function chineseTokensAll(text) { return segment.doSegment(text, { simple: true }) || []; }
function chineseTokensNoStop(text, stopSet) {
  const segs = chineseTokensAll(text);
  return segs.filter(w => w && !stopSet.has(w) && !/^\d+$/.test(w));
}

// char n-gram fallback
function charShingles(str, n = 2) {
  const s = winkUtils.string.removePunctuations(str || '').replace(/\s+/g, '');
  const out = []; for (let i = 0; i <= s.length - n; i++) out.push(s.slice(i, i + n));
  return out;
}

// ---------------- similarities ----------------
function cosineSimFromVectors(vecA, vecB) {
  let dot = 0, na = 0, nb = 0;
...
function vectorizeTfidf(tfidf, idx) { const terms = tfidf.listTerms(idx); const v = Object.create(null); for (const t of terms) v[t.term] = t.tfidf; return v; }

function maxTfidfCosineSimilarityNatural(currentTokens, historyTokenLists) {
...
function maxTfidfCosineSimilarityManual(currentTokens, historyTokenLists) {
  ...
  const tfs = docs.map(tokens => {
    const tf = Object.create(null), seen = new Set();
    for (const t of tokens) { tf[t] = (tf[t] || 0) + 1; if (!seen.has(t)) { dfs[t] = (dfs[t] || 0) + 1; seen.add(t); } }
    return tf;
  });
  const idf = Object.create(null);
  Object.keys(dfs).forEach(t => { const df = dfs[t]; idf[t] = Math.log(1 + (N / (1 + df))); });
  function vecOf(i) { const tf = tfs[i], v = Object.create(null); for (const [t, f] of Object.entries(tf)) v[t] = (1 + Math.log(f)) * (idf[t] || 0); return v; }
  const curVec = vecOf(0);
  let maxCos = 0; for (let i = 1; i < docs.length; i++) maxCos = Math.max(maxCos, cosineSimFromVectors(curVec, vecOf(i)));
  return maxCos;
}

function jaccardMax(currentTokens, historyTokenLists) {
...
function overlapCoefMax(currentTokens, historyTokenLists) {
  if (!historyTokenLists.length || !currentTokens.length) return 0;
...
function hamming32(a, b) { let x = (a ^ b) >>> 0; let c = 0; while (x) { x &= x - 1; c++; } return c; }
function simhashNearnessMax(curTokens, histTokenLists) {
...
function makeHashers(k = 64, seed = 1337) {
  ...
  return (x) => {
    ...
    for (let i = 0; i < k; i++) {
      out[i] = ((Math.imul(a[i], x >>> 0) + b[i]) >>> 0);
    }
    return out;
  };
}
function minhashSignature(tokens, k = 64, seed = 1337) {
  const hasher = makeHashers(k, seed);
  const INF = 0xffffffff >>> 0;
  const sig = new Array(k).fill(INF);
  for (const t of tokens || []) {
    const x = hash32(String(t));
    const hv = hasher(x);
    for (let i = 0; i < k; i++) { if (hv[i] < sig[i]) sig[i] = hv[i]; }
  }
  return sig;
}
...
function minhashNearnessMax(text, historyTexts, n = 3, k = 64) {
...
// BM25
function normBm25Score(raw) { const K = 8; return (isNum(raw) && raw > 0) ? (raw / (raw + K)) : 0; }
function bm25MaxManual(queryTokens, docsTokens, k1 = 1.5, b = 0.75) {
  const docs = docsTokens.filter(Array.isArray); if (!docs.length || !queryTokens.length) return 0;
  const N = docs.length, df = Object.create(null);
  const tfDocs = docs.map(tokens => {
    const tf = Object.create(null), seen = new Set();
    for (const t of tokens) { tf[t] = (tf[t] || 0) + 1; if (!seen.has(t)) { df[t] = (df[t] || 0) + 1; seen.add(t); } }
    return tf;
  });
  const docLens = tfDocs.map(tf => Object.values(tf).reduce((a,b)=>a+b,0));
  const avgdl = docLens.reduce((a,b)=>a+b,0) / N || 1;
  function scoreDoc(i) {
    const tf = tfDocs[i], dl = docLens[i] || 0; let s = 0;
...
function repeatCharRatio(text) { if (!text) return 0; const m = text.match(/(.)\1{2,}/g) || []; return clamp(m.length / Math.max(5, text.length / 10), 0, 1); }
function uniqueCharRatio(text) { if (!text) return 0; const set = new Set(text.replace(/\s+/g, '')); return clamp(set.size / Math.max(1, text.length), 0, 1); }
function digitRatio(text) { if (!text) return 0; const d = (text.match(/\d/g) || []).length; return clamp(d / text.length, 0, 1); }
function shortUtteranceScore(totalTokens, hasQP, hasLink) { const s = clamp(1 - (totalTokens / 5), 0, 1); const protect = Math.max(hasQP ? 0.6 : 0, hasLink ? 0.4 : 0); return Math.max(0, s * (1 - protect)); }

function defaultQuestionScore(text, lang, doc, punctCfg) {
...
function defaultCtaScore(text, lang, { doc, features, ruleScore }) {
  ...
  if (lang === 'eng' && doc) {
    try {
      ...
      for (const s of sents) {
        ...
        const first = terms[0]; const tags = new Set(first.tags || []);
...
function defaultUrgencyScore(text, lang, capsRatio, punctInt) { return clamp(0.6 * punctInt + 0.4 * capsRatio, 0, 1); }
function defaultAckScore(text, lang, totalTokens, semanticScore, hasQP, hasLink) {
...
function profaneScoreEn(text, tokens) {
...
function createDefaultZhDetector() {
  // 无内置关键词；仅桥接外部敏感词库/过滤器，若存在则使用
  let swfDetector = null;
  try {
    if (typeof SensitiveWordFilter?.filter === 'function') {
      swfDetector = (text) => {
        try {
      ...
      const inst = new SensitiveWordFilter();
    ...
    const tryOne = (det, s) => {
      try {
        ...
        if (r && typeof r.score === 'number') {
          score = Math.max(score, clamp(r.score, 0, 1));
          matches += r.matches || 0;
...
export class ConversationAnalyzer {
  constructor(config = {}) {
    const defaults = {
      ...
      stopwords: { en: undefined, zh: undefined },
      ...
      compliance: {
        ...
        zhDetector: undefined // function (text, tokens) => {score,matches}|number
      },
  ...
  analyze(currentText, historyTexts = [], meta = {}) {
    ...
    if (bm25Factory && tfHistTokens.length && tfCurrentTokens.length && tfHistTokens.some(t => t && t.length)) {
      try {
        ...
        if (q) { const hits = bm25.search(q) || []; if (hits.length && isNum(hits[0].score)) bm25Max = hits[0].score; }
      } catch { bm25Max = 0; }
    }
    ...
    const ctaScore = typeof fns.cta === 'function'
      ...
      : defaultCtaScore(currentText, lang, { doc: doc || nlp(''), features: { punctIntensity: result.features.punctIntensity, capsRatio: result.features.capsRatio, mentionCount }, ruleScore: ctaRuleScore });

    const urgScore = typeof fns.urgency === 'function'
      ? clamp(fns.urgency(currentText, lang, { capsRatio: result.features.capsRatio, punctIntensity: result.features.punctIntensity }) || 0, 0, 1)
      : defaultUrgencyScore(currentText, lang, result.features.capsRatio, result.features.punctIntensity);
    ...
    const lenNorm = clamp(result.features.length / 80, 0, 1);
    ...
    if (simSignals.length === 1) {
      histCompositeSim = clamp(simSignals[0], 0, 1);
    } else if (simSignals.length > 1) {
      const sorted = simSignals.slice().sort((a, b) => a - b);
      const trimmed = sorted.length > 2 ? sorted.slice(1, -1) : sorted;
      const sum = trimmed.reduce((s, v) => s + v, 0);
      histCompositeSim = clamp(sum / trimmed.length, 0, 1);
    }
    ...
    if (Array.isArray(isotonic) && isotonic.length >= 2) {
      ...
      if (p <= tbl[0].x) p = tbl[0].y;
      else if (p >= tbl[tbl.length-1].x) p = tbl[tbl.length-1].y;
      else {
  ...
  checkContextSimilarity(currentText, historyTexts) {
    let maxSimilarity = 0;
    ...
    for (const historyMsg of historyTexts || []) {
      ...
      let jaroScore = 0; try { jaroScore = natural.JaroWinklerDistance(safeCurrent, cleanHistory, undefined, true); } catch {}
  ...
  calculateEntropy(str) {
    ...
    for (let i = 0; i < l; i++) freq[str[i]] = (freq[str[i]] || 0) + 1;
    let sum = 0; for (const f of Object.values(freq)) { const p = f / l; sum -= p * Math.log2(p); }
    return sum;
  }
}
