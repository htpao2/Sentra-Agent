...
export async function buildProactiveRootDirectiveXml(payload = {}) {
  ...
  if (shortTopic) {
    defaultObjectiveLines.push(
      `围绕最近对话的大致主题「${shortTopic}」，但将这些内容视为背景，判断这轮是否适合由你主动说一句话来从新的视角或子话题切入，或者做温和的总结/收尾。`
    );
  } else {
    defaultObjectiveLines.push(
      '根据当前会话的整体氛围、节奏和情绪，判断这轮是否适合由你主动说一句话来推动气氛、从新的视角开一个相关话题，或者做温和的总结/收尾。'
    );
  }
  defaultObjectiveLines.push(
    '如果合适，请结合当前时间、背景、用户习惯与情绪，以及你的人设（RP 模式），自然延展出一个有新意的角度或话题，不要简单重复你刚才已经给出的回答，也不要再次逐字解答同一个问题。'
  );
  defaultObjectiveLines.push(
    '如果找不到有新意、对用户有价值的补充或话题延展，则保持沉默（可以输出空的 sentra-response 以表示本轮不发言）。'
  );
