...
export default async function handler(args = {}, options = {}) {
  ...
  if (!Number.isFinite(group_id)) return { success: false, code: 'INVALID', error: 'group_id 不能为空', advice: buildAdvice('INVALID', { tool: 'qq_group_ban' }) };
  if (!Number.isFinite(user_id)) return { success: false, code: 'INVALID', error: 'user_id 不能为空', advice: buildAdvice('INVALID', { tool: 'qq_group_ban', group_id }) };
  ...
  try {
    ...
    return { success: false, code: isTimeout ? 'TIMEOUT' : 'ERR', error: String(e?.message || e), advice: buildAdvice(isTimeout ? 'TIMEOUT' : 'ERR', { tool: 'qq_group_ban', group_id, user_id, duration }) };
  }
}
