...
function buildAdvice(kind, ctx = {}) {
  const personaHint = '请结合你当前的预设/人设继续作答：当修改群名失败时，要说明原因（参数/权限/机器人状态/WS 连接），给替代方案（补参数/确认管理员权限/重试）。';
  if (kind === 'INVALID') {
    return {
      suggested_reply: '我可以帮你修改群名称，但需要提供 group_id（群号）和 name（新群名）。你把这两个信息发我一下，我就继续。\n\n（请结合你当前的预设/人设继续作答）',
      next_steps: [
        '提供 group_id（群号，数字）',
        '提供 name（新群名称）',
      ],
  ...
  if (kind === 'TIMEOUT') {
    return {
      suggested_reply: '我尝试修改群名，但 QQ 侧接口没有及时响应（像是超时了）。你确认机器人与 WS 服务在线后，我可以再试一次。\n\n（请结合你当前的预设/人设继续作答）',
      next_steps: [
  ...
  return {
    suggested_reply: '我尝试修改群名，但这次失败了。常见原因是机器人权限不足或 WS 服务未连接。我可以帮你排查后再试。\n\n（请结合你当前的预设/人设继续作答）',
    next_steps: [
      '确认机器人是管理员/群主',
      '确认 group_id 正确',
      '确认 WS 服务在线后重试',
