 ...
 */
function buildSummarySystemPrompt({ chatType, botName }) {
...
function buildLinearHistoryText(historyConversations, maxChars = 8000) {
 ...
 */
export function buildContextSummaryMessages(options = {}) {
  const {
  ...
  metaParts.push(formatTimeRange({ timeStart, timeEnd }));
...
export async function compressContext(params = {}) {
  ...
  const messages = buildContextSummaryMessages({
    historyConversations,
    chatType,
    groupId,
    userId,
